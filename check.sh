...
#
# ⚠ mask_home() 只能用在 (a) 類輸出：可能被複製貼給第三方的診斷文字
# （規則的「詳細資料」、複製用的 COPY_TEXT）。
#
# 絕對不能用在 (b) 類輸出：給使用者自己看、要拿去操作的東西
# （report.html 的實際路徑、下一步要跑的指令）。這類文字遮了就等於
# 壞掉——曾經真的把「報告已產生：/home/xxx/report.html」這行也遮成
# 「/home/使用者/report.html」，使用者複製貼上會找不到檔案，因為那
# 不是要給別人看的診斷資訊，是使用者自己要用的操作指引。
#
# 新增任何 mask_home 呼叫點之前，先問：這段文字是要給「別人」看
# （AI、維護者）還是給「使用者自己」看（要照著做動作）？只有前者過這層。
# ---------------------------------------------------------------------------
mask_home() {
    sed -E \
        ...
        -e 's#/root/#~/#g' \
        -e 's#([A-Za-z]):\\Users\\[^\\[:space:]"]+#\1:\\Users\\使用者#g'
}
...
HTMLFOOT
    ...
    echo
    # 這一行是 (b) 類輸出：使用者要照這個路徑去開檔案，不是要貼給別人看的
    # 診斷資訊。遮了會壞掉——之前真的把這行也遮成「/home/使用者/report.html」，
    # 使用者複製貼上會找不到檔案。不能套 mask_home，見上面的分類說明。
